 ...
 * CONSTANTS TO CHANGE
 */
const SPREADSHEET_ID = 'CHANGE_ME'; // Change this to your spreadsheet ID
...
function duplicateBirthdayEntry(birthday) {
  ...
  const fullBirthdayEventName = birthdayNameWrapper(birthdayName, birthdate);
...
function careful_purge_duplicateBirthdayEntry(birthday) {
  const [birthdayName, birthdate] = birthday; 
  const fullBirthdayEventName = birthdayNameWrapper(birthdayName, birthdate);
  ...
  try {
    ...
    for(const item of events.items) {
      if(item.summary === fullBirthdayEventName) {
        console.log(`Duplicate found: ${item.summary}, deleting...`);
 ...
 */
function birthdayNameWrapper(name, birthdate) {
  if(birthdate) {
    const [month, day, year] = birthdate.split('/');
    return `${name}'s Birthday! Born in ${year}.`;
  }
...
/** 
 * Controller for Purging
 */
function careful_mainPurge() {
  console.log('Kicked off processing')
  const birthdayArray = readBirthdays();
  ...
  for(const birthday of birthdayArray) {
    ...
    console.log(`Veryfing existence of ${birthdayNameWrapper(birthday[0])}...`);
    careful_purge_duplicateBirthdayEntry(birthday);
  }
}
